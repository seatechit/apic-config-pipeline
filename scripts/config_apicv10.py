...
############################################
	...
	if response.status_code != 201:
		for message in response.json()['message']:
			if 'already exists with the same name' in message:
				found = True
...
###############################################################
	...
	print(info(11) + "###############################################################")
	print(info(11) + "# Step 11 - Get the IBM API Connect Provider Adefault-idp-1PI Bearer Token #")
	print(info(11) + "###############################################################")
